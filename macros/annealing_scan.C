...
void annealing_scan()
{
    std::map<std::string, double> status_to_quantity = database::status_to_annealing_time;
    ...
    std::vector<std::string> status_for_scan;
    for (auto [current_board, status_list] : database::board_to_status_list)
    {
        for (auto current_status : status_list)
        {
            if ((current_status.find("NEW") != std::string::npos))
            {
                boards_for_scan.push_back(current_board);
                status_for_scan.push_back(current_status);
                break;
            }
            if ((current_status.find("TIFPA") != std::string::npos))
            {
                boards_for_scan.push_back(current_board);
                status_for_scan.push_back(current_status);
                break;
            }
            if ((current_status.find("PANN") != std::string::npos))
            {
                boards_for_scan.push_back(current_board);
                status_for_scan.push_back(current_status);
                break;
            }
            if ((current_status.find("SANN") != std::string::npos))
            {
                boards_for_scan.push_back(current_board);
                status_for_scan.push_back(current_status);
                break;
    ...
    _TMultiGraph["_HealIV"] = new TMultiGraph();
    _TMultiGraph["_HealDCR"] = new TMultiGraph();
    //
    auto iTer = -1;
    for (auto [current_sensor, current_channels] : database::sensor_to_channels)
    {
        ...
        _TGraphErrors[vbd_key_name]->GetXaxis()->SetTitle("Anneal time (s)");
        _TGraphErrors[vbd_key_name]->GetYaxis()->SetTitle("Voltage (V)");
        //  Vact in fixed voltage
        auto vact_key_name = TString(current_sensor) + "_HealIV";
        ...
        _TGraphErrors[vact_key_name]->GetXaxis()->SetTitle("Anneal time (s)");
        _TGraphErrors[vact_key_name]->GetYaxis()->SetTitle(Form("ln(I_{dark} @ %.1fV)", reference_Vbd[current_sensor] + 3));
        //  Vact in overvoltage
        auto vact_VoV_key_name = TString(current_sensor) + "_HealDCR";
        ...
        _TGraphErrors[vact_VoV_key_name]->GetXaxis()->SetTitle("Anneal time (s)");
